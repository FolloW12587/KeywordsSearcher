...
class AppView(ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return queryset.filter(app__in=self.request.user.profile.apps_allowed.all())
...
class AppGroupView(ReadOnlyModelViewSet):
    """View for output list of app Groups"""

    queryset = models.AppGroup.objects.all()
    ...
    def get_queryset(self):
        ...
        if not self.request.user.has_perm("kwfinder.can_see_all"):
            if not hasattr(self.request.user, "profile"):
                ...
                queryset = queryset.filter(app__in=self.request.user.profile.apps_allowed.all())
        ...
        if has_data and (has_data == "1") and start_date and end_date:
            ...
            if data_app_ids:
                filter_ids = filter_ids.filter(app__id__in=data_app_ids.split(","))

            filter_ids = filter_ids.values_list("keyword", flat=True).distinct()

            queryset = queryset.filter(id__in=list(filter_ids))
...
class AppPositionScriptRunDataView(ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if date:
            queryset = queryset.filter(run__started_at__gte=f"{date} 00:00:00", run__started_at__lte=f"{date} 23:59:59")
        return queryset
...
class KeitaroDailyAppDataView(ReadOnlyModelViewSet):
    ...
    filterset_fields = {"app__id": ["exact", "in"], "date": ["exact", "gte", "lte"]}
    ordering_fields = [
        "date",
    ]
...
class ConsoleDailyDataView(ReadOnlyModelViewSet):
    ...
    filterset_fields = {"app__id": ["exact", "in"], "keyword__id": ["exact", "in"], "date": ["exact", "gte", "lte"]}
    ordering_fields = [
        "date",
    ]
...
class ASOWorldOrderKeywordDataView(ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return queryset.filter(order__app__in=self.request.user.profile.apps_allowed.all())
...
class ConsoleDataApiView(APIView):
    ...
    def post(self, request) -> Response:
        serializer = serializers.ConsoleDailyAPIDataSerializer(data=request.data, many=True)
        ...
        response_data = {"updated": 0, "not_found": 0, "created": 0}
...
def groupsAnalytics(request):
    ...
    return render(request, "kwfinder/groups_stats.html", {"groups": groups, "regions": regions})
