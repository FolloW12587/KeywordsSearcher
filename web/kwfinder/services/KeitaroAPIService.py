...
class KeitaroAPIService:
    ...
    def __init__(self) -> None:
        assert os.getenv("KEITARO_API_KEY"),\
            "KEITARO_API_KEY environment variable is not set!"
        assert os.getenv("KEITARO_HOST"),\
            "KEITARO_HOST environment variable is not set!"
    ...
    def getReport(self, campaign_id_list: list[str], date_from: date, date_to: date) -> list[dict]:
        ...
        Returns:
            list[dict]: results
        """
        logger.info(f"Creating request body for keitaro report for campaigns {campaign_id_list}, \
                    for period from {date_from}, to {date_to}.")
        data = {
            "range": {
                "from": date_from.strftime(r"%Y-%m-%d"),
                "to": date_to.strftime(r"%Y-%m-%d"),
                "timezone": "UTC"
            },
            "columns": [],
            "metrics": [
            ...
            "grouping": [
                models.KeitaroDailyAppData.DATE_FIELD_NAME,
                models.KeitaroDailyAppData.CAMPAIGN_ID_FIELD_NAME
            ],
            "filters": [
                {
            ...
            "sort": [
                {
                    "name": models.KeitaroDailyAppData.DATE_FIELD_NAME,
                    "order": "asc"
                }
            ],
            "summary": True
    ...
    def __buildReport(self, data: dict[str, Any]) -> list[dict]:
        ...
        while offset < total:
            ...
            if r.status_code != 200:
                logger.error(
                    f"Request to {endpoint} ended with {r.status_code} code: {r.text}")
                return rows
